...
- **6.1**: 介绍编译器的基本概念，编译器的生效环节，编译器的不同阶段
- **6.2**: 编译器的基本结构和工作原理，词法分析-语法分析-代码生成，使用图示展示工作流程

#### 7 常见编译器的介绍

- **7.1**: 介绍 GCC 和 LLVM 的基本信息，分别演示一下 `hello.c` 的编译过程，两种编译器的区别

#### 8 编译器的实践

- **8.1**: 演示 `rvcc` 的环境配置，这个迷你编译器的特点，`rvcc.h` 文件中的一些结构体
- **8.2**: 录制 `rvcc` 具体的 `debug` 过程，生成的汇编文件如何进行测试

#### 9 开源社区 + 学习资料

- **9.1**: 介绍编译器相关学习资料，如何为 LLVM 社区贡献代码
